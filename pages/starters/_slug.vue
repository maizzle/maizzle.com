    ...
    <main class="grid grid-cols-12 h-full">
      ...
      <div class="col-span-12 lg:col-span-8 grid grid-cols-1 lg:grid-cols-12 max-w-[65ch] lg:max-w-full mx-auto lg:m-0">
        <article class="col-span-9 2xl:col-span-7 px-4 sm:px-8 pb-12 mt-4 xl:mt-8 space-y-8">
          ...
          <nuxt-content
            :document="starter"
            class="prose max-w-[75ch] prose-slate prose-h2:mt-[1em] prose-headings:text-slate-800 prose-p:text-slate-600"
          />
          <hr>
...
export default {
  async asyncData({ $content, params, error }) {
    const [starter] = await $content({ deep: true }).where({ path: `/starters/${params.slug}` }).fetch()

    const [prev, next] = await $content('starters')
    .only(['title', 'slug'])
    .sortBy('createdAt', 'asc')
    .surround(`/starters/${params.slug}`)
    .fetch()
  ...
  head() {
    return {
      title: this.starter.title + ' Starter | Maizzle',
      meta: [
        {
          hid: 'og:title',
          name: 'og:title',
          content: this.starter.title + ' Starter | Maizzle',
        },
        {
          hid: 'twitter:title',
          name: 'twitter:title',
        ...
        },
        {
          hid: 'description',
          name: 'description',
          content: this.starter.description
        },
        {
          hid: 'og:description',
          name: 'og:description',
