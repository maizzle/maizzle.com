...
i18n is a numeronym that stands for _i(nternationalizatio)n_. It is used to refer to the practice of designing and developing content that can be easily localized for different languages and regions.

See our [internationalization guide](/docs/development/i18n) for more information on i18n in Maizzle projects.
...
A ghost table in HTML email development is a table structure split into two parts, each commented-out through Outlook conditional comments. Real HTML is added in between the two parts, for modern email clients.

```html
<!--[if mso]>
<table align="center" cellspacing="0" cellpadding="0" role="none" style="width: 576px">
  <tr>
    <td>
<![endif]-->
...
Ghost tables are primarily used to control the layout width in old Outlook on Windows when coding hybrid or spongy HTML emails. Maizzle uses this technique for the structural components like `Container`, `Row` or `Column`.
...
This can increase the risk of Gmail clipping, as it adds a few extra KB to your email's size.
::

In Maizzle, you can just use the `<Preheader>` component to add preview text to an email:

```vue [emails/example.vue]
<template>
  <Preheader>Your preview text here</Preheader>
  <!-- rest of the email -->
</template>
```
...
See the [Font](/docs/components/font) component for how to use them in Maizzle.
...
As a rule of thumb, always make sure all images have descriptive alt text and that the email is still readable and engaging without images. You can even go the extra mile and style the alt text, for better accessibility and visual appeal when images are blocked.

## Bulletproof buttons

_Bulletproof_ button is a slightly misleading term used in HTML email development that describes buttons that look and work the same in all email clients.

The "bulletproof" part came to be because of Outlook on Windows, which does not support padding on inline elements such as `<a>` tags. To fix this, [VML code](https://learn.microsoft.com/en-us/windows/win32/vml/web-workshop---specs---standards----introduction-to-vector-markup-language--vml-) is used to create a fully clickable button that looks and works mostly the same in Outlook.
...
<div>
  ...
  <v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" href="https://maizzle.com" style="height: 46px; width: 120px; v-text-anchor:middle;" arcsize="10%" stroke="f" fillcolor="#6366f1">
    ...
    <center>
  <![endif]-->
  <a href="https://maizzle.com" style="...">
    Read more
  </a>
  <!--[if mso]>
    </center>
...
The term is misleading because buttons that use <abbr title="Vector Markup Language">VML</abbr> are not accessible to screen readers - they are rendered as images by the Word rendering engine in Outlook. Also, the VML `href` is not keyboard-navigable, creates duplication, and most email service providers do not support tracking clicks on it.

A better name for these buttons would be _VML buttons_.

## Bulletproof backgrounds

The Word rendering engine, used in Outlook (classic) for Windows, does not support CSS background images in HTML emails. Similar to buttons, VML code is used to work around this. _Bulletproof_ makes sense here.

Here's an example:

```html
<table cellpadding="0" cellspacing="0" role="none" style="width: 600px">
  <tr>
    <td style="background-image: url('https://picsum.photos/600/400'); background-repeat: no-repeat; background-size: cover;">
      <!--[if mso]>
...
While VML does help create more consistent designs across email clients, it's important to note that it is not accessible to screen readers because it is rendered as an image by the Word rendering engine in Outlook (classic).
