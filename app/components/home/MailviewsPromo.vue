...
const columns = [
  [
    '/images/mailviews/2-1.jpg',
    '/images/mailviews/2-2.jpg',
    '/images/mailviews/2-3.jpg',
  ],
  [
    '/images/mailviews/1-1.jpg',
    '/images/mailviews/1-2.jpg',
    '/images/mailviews/email-5.jpg',
  ],
  [
    '/images/mailviews/3-1.jpg',
    '/images/mailviews/3-2.jpg',
    '/images/mailviews/3-3.jpg',
    '/images/mailviews/4-1.jpg',
  ],
  [
    '/images/mailviews/4-1.jpg',
    '/images/mailviews/frame-13.jpg',
    '/images/mailviews/4-1.jpg',
  ],
]

// Final resting Y per column — creates the masonry stagger at rest
// Even columns slide down into position, odd slide up
const REST_Y = [80, 15, 6, -80] as const
const ANIM_DELTA = 60

const colEls = ref<(HTMLElement | null)[]>([null, null, null, null])
const containerEl = ref<HTMLElement | null>(null)
let observer: IntersectionObserver | null = null

function setColRef(el: unknown, i: number) {
  colEls.value[i] = el as HTMLElement | null
}

onMounted(() => {
  // Reduced-motion: skip the slide-up/fade entrance, settle straight to the
  // resting masonry layout (REST_Y is position, not motion, so it's kept).
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    colEls.value.forEach((el, i) => {
      if (el) gsap.set(el, { y: REST_Y[i] ?? 0, opacity: 1 })
    })
    return
  }
  ...
  colEls.value.forEach((el, i) => {
    if (!el) return
    const rest = REST_Y[i] ?? 0
    const startY = i % 2 === 0 ? rest - ANIM_DELTA : rest + ANIM_DELTA
...
<template>
  <ContainerAlt :bottom-border="bottomBorder">
    <div class="pt-20 sm:pt-28 pb-24 px-4 sm:px-0 bg-background">
      <div class="relative overflow-hidden rounded-2xl bg-gray-900 text-white selection:bg-white selection:text-gray-900">
        <div class="absolute inset-0 bg-repeat opacity-[0.05]" :style="{ backgroundImage: 'url(/stripes.svg)' }" />
        <div class="relative grid lg:grid-cols-[5fr_6fr] min-h-[560px]">

          <div class="p-8 sm:p-14 flex flex-col justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="165" height="24" viewBox="0 0 165 24" fill="none" class="text-white">
            ...
            <a
              ...
              target="_blank"
              class="mt-8 self-start inline-flex items-center gap-2 h-11 px-6 text-sm/5 font-semibold text-gray-900 bg-white hover:bg-white/90 rounded-md transition-colors"
            >
              Explore Mailviews
          ...
          <!-- On mobile: min-h gives cards visible space; on lg: stretches to row height -->
          <div class="relative min-h-[260px] lg:min-h-0">
            <div ref="containerEl" class="absolute inset-y-0 left-0 -right-[2%] flex gap-2">
              <div
                v-for="(col, ci) in columns"
                :key="ci"
                :ref="(el) => setColRef(el, ci)"
                class="flex-1 flex flex-col gap-2 opacity-0"
              >
                <div
                  v-for="(src, ki) in col"
                  :key="ki"
                  class="w-full shrink-0 rounded-md overflow-hidden"
                >
                  <img :src="src" alt="" class="w-full h-auto block" loading="lazy" />
                </div>
              </div>
            </div>
          </div>
