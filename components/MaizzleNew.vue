  ...
  methods: {
    play() {
      ...
      this.typeItInstance = new TypeIt(this.$refs.demo, {
        ...
        .type('<span class="text-emerald-500 ml-2 mr-4">?</span> Project directory name (maizzle) ', { instant: true })
        .pause(2000)
        .delete(10, {instant: true})
        .type('<span class="text-indigo-500">maizzle</span>', {instant: true})
        .break()

        .type('<span class="text-emerald-500 ml-2 mr-4">?</span> Do you want to use a custom Starter (y/N) ', { instant: true })
        .pause(2000)
        .delete(6, {instant: true})
        .type('<span class="text-indigo-500">No</span>', {instant: true})
        .break()

        .type('<span class="text-emerald-500 ml-2 mr-4">?</span> Install NPM dependencies (Y/n) ', { instant: true })
        .pause(2000)
        .delete(6, {instant: true})
        .type('<span class="text-indigo-500">Yes</span>', {instant: true})
        .break()

        .type('<span class="text-indigo-500 ml-2 mr-4 inline-block animate-spin-fast">|</span> Crafting new Maizzle project in ~/Users/cosmin/maizzle', { instant: true })
        .pause(3000)
        .delete('.animate-spin-fast', {instant: true})
        .type('<span class="text-emerald-500 ml-2 mr-4">✓</span> Cloned Git repository', {instant: true})
        .break()

        .type('<span class="text-indigo-500 ml-2 mr-4 inline-block animate-spin-fast">|</span> Installing NPM dependencies', { instant: true })
        .pause(3000)
        .delete('.animate-spin-fast', {instant: true})
        .type('<span class="text-emerald-500 ml-2 mr-4">✓</span> Installed NPM dependencies', {instant: true})
        .break()

        .type('<span class="text-emerald-500 ml-2 mr-4">✓</span> Maizzle project initialized', { instant: true })
        .break()
        .type('<span class="text-indigo-500 ml-2 mr-4">i</span> Now `cd maizzle` and start building your emails', {instant: true})
        .go()
    },
    replay() {
