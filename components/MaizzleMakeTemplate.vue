  ...
  methods: {
    play() {
      ...
      this.typeItInstance = new TypeIt(this.$refs.demo, {
        ...
        .pause(1000)
        .type('maizzle make:template')
        .pause(1000)
        .break()

        .type('<span class="text-emerald-500 ml-2 mr-4">?</span> File name (template.html) ', { instant: true })
        .pause(2000)
        .delete(16, {instant: true})
        .type('<span class="text-indigo-500">template.html</span>', {instant: true})
        .break()

        .type('<span class="text-emerald-500 ml-2 mr-4">?</span> Directory to place it in (src/templates) ', { instant: true })
        .pause(2000)
        .delete(16, {instant: true})
        .type('<span class="text-indigo-500">src/templates</span>', {instant: true})
        .break()

        .type('<span class="text-emerald-500 ml-2 mr-4">✓</span> Created new template in ~/Users/cosmin/maizzle/src/templates/template.html', { instant: true })
        .go()
    },
    replay() {
