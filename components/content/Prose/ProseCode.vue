<template>
  <div>
    <div
      ...
      class="frame flex justify-between items-center px-6 border rounded-t-md backdrop-blur-[3px] h-10"
      :class="[
        `frame-${frameType}`,
      ]"
    >
      ...
      <span
        v-if="filename"
        class="h-10 flex items-center border-b border-indigo-600 -mb-px text-xs"
      >{{ filename }}</span>
      <button
    ...
    <div
      ref="cw"
      class="shiki mb-6 border rounded-md backdrop-blur-[3px]"
      :class="[{
...
const shouldBlurUnhighlightedLines = computed(() => props.meta.split(' ').includes('blur'))
